...
class Payment {
    ...
    static async createPaymentRecord(method, transactionId, status, amount, studentId) {
        ...
        // Marcar cualquier pago anterior como no actual (is_current = false)
        const previousPaymentsQuery = `
            UPDATE payments 
            SET is_current = false 
            WHERE subscription_id = ? AND is_current = true
        `;
        await db.query(previousPaymentsQuery, [subscriptionId]);

        // Registrar el nuevo pago en la tabla payments
        const query = `
